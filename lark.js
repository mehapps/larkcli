...
function snapshot(dev) {
  ...
  const settings = {};
  for (const k of ['voice_mode', 'voice_level', 'reverb', 'eq', 'noise_cancel',
                   'indicator', 'mic_recognition', 'auto_power_off', 'perf_mode']) {
...
function printStatus(s) {
  ...
  for (const [k, v] of Object.entries(s.settings)) {
    console.log(`  ${k.padEnd(w)}  ${v.join(' ') || '(empty)'}`);
  }
  console.log();
}
...
if (cmd === 'raw') {
  ...
  console.error(`usage: lark [status|watch|raw <op hex> [args]] [--json]\n       lark --selftest`);
  process.exit(1);
}
